    ...
    case '1':
      ...
      //1. Za uneseni cijeli broj u polje A veći od 2 i manji od 10M ispiši da li je prim (prosti) broj ili ne.
        ...
        if(broj < 2 || broj > 10000000){
            rezultat.innerHTML='broj nije u rasponu od 2 do 10M';
            return;
        }
     let prim = true;
...
// kraj rješavanje 1. zadatak
    ...
    case '2':
      ...
      //savrseni broj je broj koji je jednak zbroju svojih djelitelja


      const broj2 = parseInt(b);

      if(!broj2){
            rezultat.innerHTML='Nije unesen broj'   
      }
      if(broj2 < 2 || broj2 > 1000){
        rezultat.innerHTML='broj nije u rasponu od 2 do 1000';
        return;
      }
      savrsenBroj = 0;
      for (let i = 1; i < broj2; i++){
        if(broj2 % i === 0){
          savrsenBroj += i;
        }
      }
      if(savrsenBroj === broj2){
    ...
    for(let i=0; i<p.length / 2; i++){
        if(a[i] !== p[p.length -1 - i]) {
            palindrom = false;
            break;
        }
    }
    rezultat.innerHTML = palindrom ? 'Riječ je palindrom' : 'Riječ nije palindrom';
